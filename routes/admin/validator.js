...
module.exports = {
    ...
    requirePassword: check('password').trim().isLength({ min: 4, max: 20 }).withMessage('Must be between 4 and 20 charecters'),
    ...
    requireValidPasswordForUser :  check('password').trim().custom(async (password, {req}) => {
        ...
        const valPassword = await usersRepo.comparePassword(
            user.password,
            password
        )
        if(!valPassword) {
